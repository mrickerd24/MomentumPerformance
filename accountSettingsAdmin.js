...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    if (userSnap.exists()) {
      ...
      // Role guard — only admins should be here
      if (data.role !== "admin") {
        window.location.href = "index.html";
        return;
      }
      ...
      document.getElementById("skateCanClubNumber").value = data.skateCanClubNumber || "";
    }
  } catch (error) {
...
form.addEventListener("submit", async (e) => {
  ...
  // Clear errors
  ["name", "lastName", "emailAddress", "phoneNumber", "skateCanClubNumber", "password", "passwordConfirmation"].forEach(id => {
  ...
  const skateClubNumVal = document.getElementById("skateCanClubNumber").value.trim();
  ...
  try {
    ...
    await updateDoc(userRef, {
      ...
      skateCanClubNumber: skateClubNumVal
    });
